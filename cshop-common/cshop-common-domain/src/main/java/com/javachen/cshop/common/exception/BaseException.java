 ...
 */
@Getter
public class BaseException extends RuntimeException implements ErrorCodeAware {

    private ErrorCodeAware errorCodeAware;

    //直接接收EmBusinessError的传参用于构造业务异常
    public BaseException(ErrorCodeAware errorCodeAware) {
        super(errorCodeAware.getMessage());
        this.errorCodeAware = errorCodeAware;
    }

    //接收自定义errorMsg的方式构造业务异常
    public BaseException(ErrorCodeAware errorCodeAware, String message) {
        super(message);
        this.errorCodeAware = errorCodeAware;
