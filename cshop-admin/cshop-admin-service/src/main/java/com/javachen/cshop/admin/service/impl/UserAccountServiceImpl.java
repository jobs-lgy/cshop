...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    public User register(UserRegister userRegister) {
        ...
        if (userRepository.findByPhone(user.getPhone()) != null) {
            throw new BaseException(ErrorCode.USER_PHONE_ALREADY_EXIST);
        }

        if (userRepository.findByUsername(user.getUsername()) != null) {
            throw new BaseException(ErrorCode.USER_USERNAME_ALREADY_EXIST);
        }

        if (userRepository.findByEmail(user.getEmail()) != null) {
            throw new BaseException(ErrorCode.USER_EMAIL_ALREADY_EXIST);
        }

        if (!userRegister.getPassword().equals(userRegister.getPasswordConfirm())) {
            throw new BaseException(ErrorCode.USER_PASSWORD_NOT_EQUAL);
        }
...
//            this.stringRedisTemplate.delete(key);
    ...
    public User login(UserLogin userLogin) {
        ...
        if(isPasswordValid(user.getPassword(),userLogin.getPassword())){
            ...
            throw new BaseException(ErrorCode.USER_LOGIN_ERROR);
        }
    }
...
//        mqService.sendEmail("forgetUsername",user.getEmail(),resetPasswordUrl, user.getId(), user.getUsername());
    ...
    public User resetPassword(PasswordReset passwordReset) {
        ...
        if (codeCache == null || !codeCache.equals(token)) {
            throw new BaseException(ErrorCode.USER_TOKEN_INVALID);
        }
